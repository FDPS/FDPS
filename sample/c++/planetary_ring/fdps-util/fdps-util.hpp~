...
namespace FDPS_UTIL{
	...
	void read(){
		...
		std::string str = "";
		auto is_option = chopOptionName(str, i);
		//std::cerr<<"str= "<<str<<" is_option= "<<is_option<<std::endl;
		if(!is_option) continue;
		...
		if(index >= 0){
			...
			//std::cerr<<"A) str= "<<str
			//	 <<" options_[index].long_name_= "<<options_[index].long_name_
			//	 <<std::endl;
		    }
		    else if(i+1 < argc_){
			...
			//std::cerr<<"B) str= "<<str
			//	 <<" options_[index].long_name_= "<<options_[index].long_name_
			//	 <<" val_str_= "<<options_[index].val_str_
			//	 <<std::endl;
		    } else {
			//if(PS::Comm::getRank()==0){
			//    std::cerr<<"use default value for "<<str<<std::endl;
			//}	
		    }
		} else {
		    //if(PS::Comm::getRank()==0){
		    //	std::cerr<<str<<" is not defined."<<std::endl;
		    //}
		}
	    }
	}
