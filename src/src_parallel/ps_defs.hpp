...
namespace ParticleSimulator{
    ...
    public:
        //static S32 getBoundaryCondition(){ return getInstance().boundary_condition_; }
        //static void setBoundaryCondition(enum BOUNDARY_CONDITION bc){ getInstance().boundary_condition_ = bc;}
        ...
        static bool synchronizeConditionalBranchAND(bool local){
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
            bool global;
        ...
        static bool synchronizeConditionalBranchOR(bool local){
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
            bool global;
    ...
    /*
    // Are there any bugs ? (AT)
    inline F32 CalcSeparationSQBoxToBox(const F32vec & center0,
                                        const F32vec & half_length0,
                                        const F32vec & center1, 
                                        const F32vec & half_length1 ){
#ifdef PARTICLE_SIMULATOR_TWO_DIMENSION
        F32 dx = fabs(center0.x - center1.x);
        F32 dy = fabs(center0.y - center1.y);
        F32vec hlen = half_length0 + half_length1;
        dx = ( hlen.x < dx ) ? (dx - hlen.x) : 0.0;
        dy = ( hlen.y < dy ) ? (dy - hlen.y) : 0.0;
        return dx*dx + dy*dy;
#else
        F32 dx = fabs(center0.x - center1.x);
        F32 dy = fabs(center0.y - center1.y);
        F32 dz = fabs(center0.z - center1.z);
        F32vec hlen = half_length0 + half_length1;
        dx = ( hlen.x < dx ) ? (dx - hlen.x) : 0.0;
        dy = ( hlen.y < dy ) ? (dy - hlen.y) : 0.0;
        dz = ( hlen.z < dz ) ? (dz - hlen.z) : 0.0;
        return dx*dx + dy*dy + dz*dz;
#endif
    }
    */
    ...
    template<>
    inline unsigned long GetMSB(const unsigned long val){
        return (val>>63) & 0x1;
    }
    template<>
    ...
    template<>
    inline unsigned long ClearMSB(const unsigned long val){
    ...
    template<>
    inline unsigned long SetMSB(const unsigned long val){
    ...
    template<class Tp>
    inline F32ort GetMinBoxSingleThread(const Tp ptcl[], const S32 n){
        F32ort pos_box(ptcl[0].getPos(), ptcl[0].getPos());
        for(S32 i=1; i<n; i++){
    ...
    template<class Tp>
    inline F32ort GetMinBox(const Tp ptcl[], const S32 n){
	F32ort box_loc;
#pragma omp parallel
	{
	    F32ort box_loc_tmp;
...
#pragma omp critical
	...
	const F32vec xlow_loc = box_loc.low_;
	const F32vec xhigh_loc = box_loc.high_;
	const F32vec xlow_glb = Comm::getMinValue(xlow_loc);
	const F32vec xhigh_glb = Comm::getMaxValue(xhigh_loc);
	return F32ort(xlow_glb, xhigh_glb);
    }
    
    
    template<class Tp>
    inline F32ort GetMinBoxWithMargen(const Tp ptcl[], const S32 n){
	F32ort box_loc;
#pragma omp parallel
	{
	    F32ort box_loc_tmp;
...
#pragma omp critical
    ...
    struct LessOPForVecX{
        bool operator() (const F32vec & left, const F32vec & right) const {
