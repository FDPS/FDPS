...
namespace ParticleSimulator{
	...
	/*
	for(S32 i=0; i<Comm::getRank(); i++){
	    fout<<"n_ep_send_[i]="<<n_ep_send_[i]<<std::endl;
	    fout<<"n_sp_send_[i]="<<n_sp_send_[i]<<std::endl;
	}
	*/
	const S32 n_proc = Comm::getNumberOfProc();
	Tepj * ep_tmp;
	...
	S32 n_err = 0;
	for(S32 i=0; i<n_loc; i++){
	    if( fabs(mass_tree[i]-mass_direct[i]) > 1e-2){
		err = true;
	...
	else fout<<"CheckExchangeLocalEssentialTreeDummy(LONGCUTOFF): FAIL"<<std::endl;
	fout<<"this->n_ep_recv_disp_[n_proc]= "<<this->n_ep_recv_disp_[n_proc]<<std::endl;
	...
	for(S32 ib=0; ib<n_proc; ib++){
	    ...
	    for(S32 ip=0; ip<this->n_ep_recv_[ib]; ip++){
		if( (pos_tree_per_proc[ip]-pos_direct_per_proc[ip]).applyEach(Abs<F64>()).getMax() > tolerance && err_tmp == 0){
		    err_tmp |= 1<<1;
		    if(Comm::getRank() == 0){
		    fout<<"error 2"<<std::endl;
		    fout<<"Comm::getRank()= "<<Comm::getRank()<<" ib= "<<ib<<std::endl;
		    fout<<"(pos_tree_per_proc[ip]-pos_direct_per_proc[ip]).applyEach(Abs<F64>()).getMax()="<<(pos_tree_per_proc[ip]-pos_direct_per_proc[ip]).applyEach(Abs<F64>()).getMax()<<std::endl;
		    fout<<"ip="<<ip<<std::endl;
		    fout<<"pos_tree_per_proc[ip]="<<pos_tree_per_proc[ip]<<" pos_direct_per_proc[ip]="<<pos_direct_per_proc[ip]<<std::endl;
		    fout<<"pos_tree_per_proc[ip+1]="<<pos_tree_per_proc[ip+1]<<" pos_direct_per_proc[ip+1]="<<pos_direct_per_proc[ip+1]<<std::endl;
		    fout<<"pos_tree_per_proc[ip-1]="<<pos_tree_per_proc[ip-1]<<" pos_direct_per_proc[ip-1]="<<pos_direct_per_proc[ip-1]<<std::endl;
		    fout<<"this->n_ep_recv_[ib]= "<<this->n_ep_recv_[ib]<<" n_recv_direct[ib]="<<n_recv_direct[ib]<<std::endl;
		    }
		    err = true;
